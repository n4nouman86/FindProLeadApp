...
import type { AutoLead, CreateAutoLeadRequest } from "./types";
...
import { createAutoLead } from "./autoLeadsApi";
...
interface CreateAutoLeadDialogProps {
  open: boolean;
  onClose: () => void;
  onSaved: (lead: AutoLead) => void;
}

export default function CreateAutoLeadDialog({ open, onClose, onSaved }: CreateAutoLeadDialogProps) {
  ...
  const [verifierCompanyId, setVerifierCompanyId] = useState("");
  ...
  async function handleSubmit(event: SubmitEvent<HTMLFormElement>) {
    ...
    const request: CreateAutoLeadRequest = {
      firstName,
      lastName,
      phone,
    ...
    try {
      const saved = await createAutoLead(request);
      onSaved(saved);
      onClose();
      notify("Auto lead created successfully.");
    } catch (err) {
      const message = getErrorMessage(err, "Could not create auto lead. Please check the details and try again.");
      setError(message);
    ...
    <Dialog open={open} onClose={onClose} fullWidth maxWidth="sm">
      <Box component="form" onSubmit={handleSubmit}>
        <DialogTitle sx={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          Add Auto Lead
          <IconButton onClick={onClose} size="small" aria-label="Close add auto lead dialog">
